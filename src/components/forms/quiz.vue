<template>
  <div id="options" ref="options" class="d-flex flex-column align-items-center justify-content-center">
    <TransitionGroup name="slide-fade"  tag="ul" class="list-group">
      <li v-for="(quiz, index) in getState('quizzes').slice(getQuizState('index'), getQuizState('index')+1)" :key="index"
       class="list-group-item  quiz border shadow-sm  rounded rounded-4 p-4" :class="{'border border-1 border-danger' :getQuizState('errAlert') }">


        <div class="d-flex justify-content-between">
          <p class="mb fw-bold fst-italic text-center ">{{quiz.title}}</p>
          <span class="text-muted fs-8 d-none d-sm-block">{{getQuizState('index')+1}}/{{getQuizState('length')}}</span>
        </div>
...
<script>
  ...
  export default{
    name: 'math-question',
    computed: mapGetters(['getState', 'getQuizState']),  
    methods: {
      async answer(e){
