...
<script>
  import Question from '@/components/forms/screener-question'
  import Instruction from '@/components/misc/screener-instruction'
  ...
  export default{
    ...
    async created(){
      const { screenerQues } = this.$store.state

      if(! screenerQues.length)
